...
- **Page de remerciement** : après paiement réussi, redirection vers **`/billing/success`** (page standalone, hors dashboard) : message de remerciement, **refresh de session** et **sync du plan** depuis Stripe (API `POST /api/stripe/sync-plan`), puis redirection automatique vers le dashboard après 5 s. Si la session a été perdue (cookies après redirect Stripe), message « Paiement reçu — Reconnecte-toi » avec lien vers login.
- **Stripe Checkout** : création de session, redirection vers Stripe, **`success_url`** vers **`/billing/success?plan=...`**, `cancel_url` vers `/dashboard/billing?canceled=1`.
- **Sync plan (fallback)** : `POST /api/stripe/sync-plan` (authentifié) lit l’abonnement actif Stripe du client et met à jour `profiles.plan` et `profiles.storage_limit_bytes`. Appelé automatiquement sur la page de succès pour garantir la mise à jour même si le webhook n’a pas encore été traité ou a échoué.
