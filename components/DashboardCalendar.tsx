...
export function DashboardCalendar({
  ...
  return (
    <div className={cn("glass-card rounded-2xl border border-white/[0.08] overflow-hidden", compact && "max-w-xl", !compact && "w-full")}>
      <div className={cn("glass-card-header flex items-center justify-between", compact ? "px-3 py-2 sm:px-4" : "px-4 py-3 sm:px-5")}>
      ...
      <div className={compact ? "p-2 sm:p-3" : "p-4 sm:p-5"}>
        <div className={cn("grid grid-cols-7 gap-0.5 text-center font-medium text-muted-foreground", compact ? "text-[10px]" : "gap-1 text-xs")}>
          {["Lun", "Mar", "Mer", "Jeu", "Ven", "Sam", "Dim"].map((d) => (
            <div key={d} className={compact ? "py-0.5" : "py-1"}>{d}</div>
          ))}
        </div>
        <div className={cn("mt-1 grid grid-cols-7", compact ? "gap-0.5" : "mt-2 gap-1")}>
          {Array.from({ length: paddingDays }, (_, i) => (
            <div key={`pad-${i}`} className={cn("rounded bg-transparent", compact ? "aspect-square min-h-[28px]" : "aspect-square rounded-lg")} />
          ))}
          {days.map((day) => {
            ...
            return (
              <div
                ...
                className={cn(
                  "rounded p-0.5",
                  compact ? "min-h-[28px]" : "aspect-square rounded-lg p-1",
              ...
              >
                ...
                <div className={compact ? "mt-0.5 space-y-0.5" : "mt-0.5 space-y-0.5"}>
