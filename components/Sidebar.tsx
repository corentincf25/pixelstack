...
export function Sidebar() {
  ...
  // Rafraîchir l’avatar quand il est mis à jour (ex. depuis Paramètres) ou au retour sur l’onglet
  useEffect(() => {
  ...
  return (
    <aside className="glass-card fixed left-0 top-0 z-30 flex h-full w-[240px] flex-col rounded-none border-r border-white/[0.06] bg-[#121212]/95 backdrop-blur-[20px]">
      <div className="glass-card-header flex h-14 items-center gap-3 px-5">
        {/* Logo : dépose ton fichier en public/logo.png (nom exact, minuscules) */}
        ...
        {[
          ...
          return (
            <Link
              ...
              href={item.href}
              className={cn(
                "btn-interactive flex items-center gap-2.5 rounded-lg px-3 py-2.5 text-sm font-medium transition-colors",
                active
                  ? isYoutuber
      ...
      <div className="space-y-0.5 border-t border-border px-3 py-4">
        ...
        <button
          ...
          className="btn-interactive flex w-full items-center gap-2.5 rounded-lg px-3 py-2.5 text-sm font-medium text-muted-foreground transition-colors hover:bg-sidebar-accent/50 hover:text-foreground focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 focus-visible:ring-offset-background"
        >
